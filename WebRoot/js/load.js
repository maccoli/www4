		...
		function shenglue(){
			...
			var oPtext = $('.article_con p').each(function(){
			 	var aPtext = $(this).text();
			 		aPtext = $.trim(aPtext);
			 	// 字符中取数字// var videoid = aPtext.replace(/[^0-9]/ig,"");// var x2 = videoid.length;
...
window.onload = function(){
	var chart_c1="0";//用来判断是不是第一次触发图形插件函数
	var chart_c2="0";
	var chart_c3="0";
	// 显示数据时候要加上下面对应的@函数
		// @Tab2头像的加载 
			...
			//一级选项卡 
				...
				for(i=0;i < aLi_tab.length;i++){
					...
					aLi_tab[i].onclick = function(){
						...
						if(this.id=="xzxx_bar" && chart_c2=="0"){
							chart2();
			                chart_c2="1";
			            }
		        ...
		        function tab(x){   
		            ...
		            for(var i=0;i<aLi.length;i++){
		                ...
		                aLi[i].onclick = function(){
		                	// console.log("点击之前："+chart_c1+"、"+chart_c2+"、"+chart_c3);
		                    ...
		                    if(this.id=="mubiao_lsjl" && chart_c1=="0"){
		                    	chart1();
		                    	chart_c1="1";
		                    	// console.log("执行了1~"+chart_c1);
			                }
			                /*else if(this.id=="xzxx" && chart_c2=="0"){
			                	chart2();
			                	chart_c2="1";
			                	// console.log("执行了2~"+chart_c2);
			                }*/
			                else if(this.id=="rwqk" && chart_c3=="0"){
			                	chart3();
			                	chart_c3="1";
			                	// console.log("执行了3~"+chart_c3);
			                }
			                // console.log("点击之后："+chart_c1+"、"+chart_c2+"、"+chart_c3);
		                };
		            }
...
//@ajax：提交数据的ajax
	// @allTable删除表格效果 			// @ajax：delTr.aim 
	    ...
	    function deleteTable(del,num){
			...
			if(oTab.tBodies[0].rows.length==0){return;}
				...
				$.ajax({
					...
					//今日
						...
						$.each(jsonArray,function(i,value){
							...
							outStr_jr += (jsonCell.finish == "2") ? '<td><span class="glyphicon glyphicon-ok" style="color:green;"></span></td></tr>' : '<td><span class="glyphicon glyphicon-remove" style="color:red;"></span></td></tr>';
						})
				...
				$.ajax({
					...
					success : function(data){
						...
						$.each(jsonArray,function(i,value){
							...
							outStr_br += (jsonCell.wfinish == "2") ? '<span class="glyphicon glyphicon-ok" style="color:green;"></span></td></tr>' :'<span class="glyphicon glyphicon-remove" style="color:red;"></span></td></tr>';
						});
				...
				$.ajax({
					...
					success : function(data){
						...
						$.each(jsonArray,function(i,value){
							...
							savedFileName1=jsonCell.savedFileName;
						});
						$("#person_info tbody").html(outStr);
						// console.log("调试调试调试:"+$("#fileimg img").eq(0).attr("src"));
						if($("#fileimg img").eq(0).attr("src")=="http://115.159.58.215:8080/www4/img/tx/"){
							$("#fileimg img").eq(0).attr("src","http://115.159.58.215:8080/www4/img/tx/"+savedFileName1);
							// console.log("调试调试调试1:"+$("#fileimg img").eq(0).attr("src"));
						}
				...
				// @ajax
					...
					$.ajax({
						...
						success : function(data){
							...
							$.each(jsonArray,function(i,value){
								var jsonCell = value;*/
								console.log("服务器返回的数据：（1表示密码修改失败）："+data);
								if(data == "1"){
									alert("密码修改失败！");
								}
								else{
...
//3、课程记录
	// 显示课程记录 						// @ajax：showcourse
		// 课程的图片cPic，课程标题cTitle，课程记录的时间cTime,课程的编号cId
			$.ajax({
				url : 'showcourse',
				type : 'GET',
				success : function(data){
					alert("dangqianyonghu:"+userNowName);
					if(userNowName=="阿水12344" || userNowName=="浅伊_Erin" || userNowName=="蓝胖子。"){
						data = '{"list":[{"cId":"2","cPic":"img/p1.jpg","cTitle":"bootstrap框架","cTime":"2017/3/3"},{"cId":"1","cPic":"img/p2.jpg","cTitle":"ssss","cTime":"2017/3/3"},{"cId":"3","cPic":"img/p3.jpg","cTitle":"bootstrap框架","cTime":"2017/3/3"},{"cId":"4","cPic":"img/p4.jpg","cTitle":"bootstrap框架","cTime":"2017/3/3"}]}';
					}
					var jsonArray = JSON.parse(data).list;
					var outStr = "";
					$.each(jsonArray,function(i,value){
						var jsonCell = value;
						outStr += '<div class="con3-img"><a href="coursev_show1.jsp?idkc='+jsonCell.cId+'"><img src="' + jsonCell.cPic;
						outStr += '" alt=""><div class="con3-bg"></div><div class="con3-rec"><h1 style="color:#fff;">已学10%<br/></h1></div><div class="con3-title"><span class="3_title">' + jsonCell.cTitle;
				...
				error : function(){
					// alert("dangqianyonghu:"+userNowName);
					// alert(userNowName=="阿水12344");
					if(userNowName=="阿水12344" || userNowName=="浅伊_Erin" || userNowName=="蓝胖子。"){
						data = '{"list":[{"cid":"1","cPic":"course_infor/img/bootstrap.jpg","cTitle":"基于Bootst","cTime":"2017-05-01"},{"cId":"1","cPic":"course_infor/img/h5.jpg","cTitle":"HTML5基","cTime":"2017-05-01"},{"cId":"3","cPic":"course_infor/img/js1.png","cTitle":"JS运动基础","cTime":"2017-05-02"},{"cId":"4","cPic":"course_infor/img/css3.jpg","cTitle":"CSS3 3D特效","cTime":"2017-05-03"}]}';
					}
					else{
						data = '{"list":[]}';
					}
					...
					console.log(data);
					var jsonArrayk = JSON.parse(data).list;
					var outStrk = "";
					$.each(jsonArrayk,function(i,value){
						var jsonCell = value;
						outStrk += '<div class="con3-img"><a href="coursev_show1.jsp?idkc='+jsonCell.cId+'"><img src="' + jsonCell.cPic;
						outStrk += '" alt=""><div class="con3-bg"></div><div class="con3-rec"><h1 style="color:#fff;">已学10%<br/></h1></div><div class="con3-title"><span class="3_title">' + jsonCell.cTitle;
						outStrk += '</span><span class="badge" style="background:#f58220;">' + jsonCell.cTime;
						outStrk += '</span></div></a><a href="javascript:;" class="del_img">删除>></a></div>';
					})
					console.log("调试中："+jsonArrayk.length);
					$("#lesson_record").html(outStrk);
					//调用的函数
						onmouseShow();
						deleteDiv("lesson_record","con3-img",1);
				}
	...
	// 显示我的提问							// @ajax：Mintw.video
		//问题的编号(用于连接)twconid,问题的内容twcon，问题的时间twtime，问题的回答数videoid
			$.ajax({
				url : 'Mintw.video',
				type : 'GET',
				success  : function(data){
					...
					$.each(jsonArray,function(i,value){
						var jsonCell = value;
						outStr += '<tr><td><a href="askquestion.jsp?id=' + jsonCell.twconid + '">';
						outStr += jsonCell.twcon + '</a></td><td>' + jsonCell.twtime + '</td><td><span class="con3_pls">';
						outStr += jsonCell.videoid + '</span></td><td><a href="javascript:;">删除</a></td></tr>';
					});
				...
				error : function(){
					// data = '{"list" : [{"twconid":"123","twcon":"高度为100%；line-height:100%为甚字体不居中？","twtime":"2017/3/3","videoid":"21"},{"twconid":"1233","twcon":"高度为100%；line-height:100%为甚字体不居中？高度为100%；line-height:100%为甚字体不居中？高度为100%；line-height:100%为甚字体不居中？","twtime":"2017/3/3","videoid":"2"}]}';
					console.log("服务器被天狗吃掉了，我的提问加载失败");
				}
			})

//4、消息
	// 回答消息  							// @ajax：showanswertome
		//问题的编号twconid，问题的内容twcon，
			$.ajax({
				url : 'hfxiaoxi.video',
				type : 'GET',
				success : function(data){
					...
					$.each(jsonArray,function(i,value){
						var jsonCell = value ;
						outStr += '<tr><td><a href=askquestion.jsp?id=' + jsonCell.twconid + '>' + jsonCell.replycon + '</a></td><td><a href="javascript:;">删除</a></td></tr>';
					});
					if(jsonArray.length==0){
		...
		//消息
			/*$.ajax({
				...
				success : function(data){
					...
					// console.log(outStr);
					$("#newsBySystem tbody").html(outStr)//
					// 调用函数
						deleteTable("newsBySystem",1);
				},
				error : function(){
	...
	// 小组消息 							
		// 消息显示  						// @ajax：fagroupid.jia
			// 申请人的编号applid，申请人的头像applh，申请人的昵称 applicant
			$.ajax({
				url : 'fagroupid.jia',
				type : 'GET',
				success : function(data){
					...
					$.each(jsonArray,function(i,value){
						var jsonCell = value;
						outStr += '<tr><td><a href="##" data-src="'+jsonCell.applh+'" data-userid="'+jsonCell.applid+'">'+jsonCell.applicant+'</a>申请加入</td><td><a href="javascript:;">同意</a></td><td><a href="javascript:;">删除</a></td></tr>';
					});
					// console.log(outStr);
		...
		// 消息处理： 同意添加其为小组成员 	// @ajax：tyadd.group
			function AddMumber(){
					...
					{	that = this;
						...
						//确认添加
							if(confirm_val==true){
								...
								var json = {};
								json.applid = img_id;
								json.groupid = userNowGroup;						
								var $that = $(this);//发送请求之后this指向会变化，要先保存原值
								//@ajax ,发送申请者的
									$.ajax({
										url : 'tyadd.group',
										type : 'GET',
										data : json,//申请者的id，小组id根据session获取中的user_id，
										success:function(){
											//添加的内容
	...
	//文章 			 						// @ajax：showarticle.coo
		$.ajax({
			...
			success : function(data){
				...
				$.each(jsonArray,function(i,value){
					var jsonCell = value;
					outStr += '<tr><td><a href="xianshifenx.jsp?id='+jsonCell.articleId;
					outStr += '" style="color:#000">'+jsonCell.articleTitle+'</a></td><td><a href="javascript:;">取消收藏</a></td></tr>';
				})
				$("#sc_wenzhang tbody").html(outStr);
				deleteTable("sc_wenzhang",1);
			},
			error : function(){
				// var data = '{"list":[{"artid":"123","article":"hjdjdjjd基督教基督教基督教基督教觉得"},{"artid":"12223","article":"JSJSJSJSJSJS督教觉得"}]}';
				console.log("服务器被天狗吃掉了，加载文章失败~");
			}
		});
	
	//课程 			 						// @ajax：showcourse.coo
		$.ajax({
			url : 'showcourse.coo',
			type : 'GET',
			success : function(data){
				...
				$.each(jsonArray,function(i,value){
					var jsonCell = value;
					outStr += '<tr><td><a href="coursev_show1.jsp?id='+jsonCell.courseId;
					outStr += '" style="color:#000">'+jsonCell.coursetile+'</a></td><td><a href="javascript:;">取消收藏</a></td></tr>';
				})
				$("#sc_kecheng tbody").html(outStr);
				deleteTable("sc_kecheng",1);
			},
			error : function(){
				// var data = '{"list":[{"courseId":"123","course":"hjdjdjjd基督教基督教基督教基督教觉得"},{"courseId":"12223","course":"JSJSJSJSJSJS督教觉得"}]}';
				console.log("服务器被天狗吃掉了，加载课程失败~");
			}
		});
	
	//问题 			 						// @ajax：showquestion.coo
		$.ajax({
			url : 'showquestion.coo',
			type : 'GET',
			success : function(data){
				var jsonArray = JSON.parse(data).list;
				var outStr  = "";
				$.each(jsonArray,function(i,value){
					var jsonCell = value;
					outStr += '<tr><td><a href="7_share_4-7.html?id='+jsonCell.quetid;
					outStr += '" style="color:#000">'+jsonCell.question+'</a></td><td><a href="javascript:;">取消收藏</a></td></tr>';
				})
				$("#sc_wenti tbody").html(outStr);
				deleteTable("sc_wenti",1);
			},
			error : function(){
				// var data = '{"list":[{"quetid":"123","question":"hjdjdjjd基督教基督教基督教基督教觉得"},{"quetid":"12223","question":"JSJSJSJSJSJS督教觉得"}]}';
...
//6、分享
		...
		//文章编号articleId，文章标题articleTitle，文章发布时间articleTime，文章点赞数articleLikes
			$.ajax({
				...
				success : function(data){
					...
					$.each(jsonArray,function(i,value){
						var jsonCell = value;
						outStr += '<tr><td><a href="xianshifenx.jsp?id='+jsonCell.articleId+'">'+jsonCell.articleTitle+'</a></td><td>'+jsonCell.articleTime+'</td><td>'+jsonCell.articleLikes+'</td><td><a href="javascript:;">删除</a></td><td><a href="##">修改</a></td></tr>';
					});
					$("#myshare").html(outStr);
					deleteTable("myshare",3);
				...
				error : function(){
					/*var data = '{"list":[{"articleId":"123","shareTitle":"hjdjdjjd基督教基督教基督教基督教觉得","shareTime":"2017/4/6"},{"myshareId":"12ss3","shareTitle":"hjaaaaa觉得","shareTime":"2017/4/6"},{"myshareId":"123","shareTitle":"hjdfffff得","shareTime":"2017/4/6"}]}';*/
		...
		// 作者的编号writerId，作者的头像savedFileName，作者的名字writerName，文章的编号articleId，文章的标题articleTitle，文章的发布时间articleTime，文章的浏览量articleViews，文章的点赞量articleLikes，文章收藏数articleCollected，文章的内容articleCon
			$.ajax({
				...
				success : function(data){
					...
					$.each(jsonArray,function(i,value){
						var jsonCell = value;
						outStr += '<div class="con"><div class="con_left"><div class="writer_tx"><a href="otherCenter.jsp?id1=';
						...
						outStr += '</h2></div></div><div class="con_right"><a href="xianshifenx.jsp?id2=';
						...
						outStr += '</span></p></div><div class="article_con"><p>'+jsonCell.articleCon.replace(/<[^>]*>/g,"")+'</p></div></a></div></div>';
						// console.log("jsonCell.articleCon:"+jsonCell.articleCon);
						// console.log("jsonCell.articleCon:"+jsonCell.articleCon.replace(/<[^>]*>/g,""));
					})
	...
	//关注管理显示		 		 			// @ajax：showGZYH	
		// 被关注用户编号userid，被关注的头像savedFileName，被关注的昵称nickname
			$.ajax({
				url : 'showatten.atten',
				type : 'GET',
				success : function(data){
					...
					$.each(jsonArray,function(i,value){
						var jsonCell = value;
						outStr += '<tr><td><a href="otherCenter.jsp?id='+jsonCell.userid+'"><img src="img/tx/'+jsonCell.savedFileName+'" alt=""><span>'+jsonCell.nickname+'</span></a></td><td><a href="javascript:;">取消关注</a></td></tr>';
					});
					$("#gz_user tbody").html(outStr);
					deleteTable("gz_user",1);
				},
				error : function(){
					// var data = '{"list":[{"userid":"1","savedFileName":"t1.jpg","nickname":"阿水12344"},{"userid":"2","savedFileName":"t2.jpg","nickname":"彩莲12344"}]}';
...
//7、我的小组
	// 基本信息 及 信息修改 、任务情况   	// @ajax：showgroup.group
		//小组的编号groupid，小组名称groupname，小组图片groupp，阶段目标groupaim，阶段内容groupaims，，小组荣誉grouphonor,
		//小组成员teamMember(这个数据的格式还是字符串："t1.jpg,t2.jpg,t3.jpg")
		//当前用户的头像照片userNowSrc（取值：自己的src，他人的src，undefined）
		//今日目标tTarget，本周目标wTarget
			$.ajax({
				url : 'showgroup.group',
				type : 'POST',
				success : function(data){
					var jsonArray = JSON.parse(data).list;//小组的信息
					var jsonArrayA = JSON.parse(data).list2;//每个成员的信息
					// console.log(jsonArrayA);
					// console.log(jsonArrayA==null);
					// console.log(jsonArrayA==undefined);

					var outStr = "";//第三页
					var outStr2 = "";//第一页
					var outStr3 = "";//第二页
					var outStr4 = "";//小组的成员头像信息
					if(jsonArrayA==null){
						//当前用户还没有加入小组
						console.log("当前用户还没有加入小组~");
						var outStrq = '<div style="width:500px;height:500px;margin-top:200px;margin-left:100px;">您还没有假如任何小组快去<a href="teams.jsp">加入</a>把~</div>';
						$("#con7").html(outStrq);
					}
					else{
						//当前用户加入了小组
						$.each(jsonArray,function(i,value){
							...
							// alert("222"+jsonArrayA.length);
							console.log("当前用户加入了小组~");
							//显示1、3页
								var ifcaptain = "0";
								//@list2的遍历
								$.each(jsonArrayA,function(j,valuej){
									var jsonCellA = valuej;
									//在list2中找到当前用户的信息，查看他的captain属性,
									if(jsonCellA.nickname == userNowName){
										ifcaptain = jsonCellA.captain;
										outStr4 += '<div class="xz_zz zp"><img src="img/tx/'+jsonCellA.savedFileName+'" alt=""></div>';
									}
									else{
										outStr4 += '<div class="xz_cy zp"><img src="img/tx/'+jsonCellA.savedFileName+'" alt=""><div class="xz_cy_men"><a href="javascript:;">删除</a></div></div>'
									}
								});
								if(ifcaptain == "1"){
									$("#div_xzxx").css("display","block");
									console.log("当前用户是组长");
									outStr += '<tr><th>小组名称：</th><td><input class="form-control" id="groupname" type="text" value="'+jsonCell.groupname+'"  disabled="disabled"></td></tr>';
									outStr += '<tr><th>小组成员：</th><td id="xz_cy">';
									outStr += outStr4;
									outStr += '<tr><th>阶段目标：</th><td><input type="text" id="groupaim" placeholder="输入阶段目标" class="form-control" value="'+jsonCell.groupaim+'"></td></tr><tr><th>阶段内容：</th><td><textarea placeholder="请填写阶段目标" style="width:350px" rows="7" id="team_work">'+jsonCell.groupaims+'</textarea></td></tr><tr><th>小组荣誉：</th><td><textarea placeholder="请填写个人荣誉" style="width:350px" rows="5" id="team_honor">'+jsonCell.grouphonor+'</textarea></td></tr><tr><th></th><td><input type="submit" value="提交" id="XZZLbtn" class="btn btn-success" style="margin-left:300px;" ></td></tr>';
									...
									onmouseShow2();
									deleteDiv("xz_cy","xz_cy",0);
									if($("#fileimg2 img").eq(0).attr("src")=="http://115.159.58.215:8080/www4/img/tx/"){
										$("#fileimg2 img").eq(0).attr("src","http://115.159.58.215:8080/www4/img/tx/"+jsonCell.groupp);
										console.log("调试调试调试2:"+$("#fileimg img").eq(0).attr("src"));
									}


									//修改数据的ajax 						// @ajax：updatagroup.group
									$("#XZZLbtn").click(function(){
										var groupid = userNowGroup;
										var groupaim = $("#groupaim").val();
										var groupaims = $("#team_work").val().replace('<br />','/n');
										var grouphonor = $("#team_honor").val();
										console.log("groupname:"+groupname+" groupaim:"+groupaim+" groupaims:"+groupaims+" grouphonor:"+grouphonor);
										//发送数据
											var json = {};//新建一个json对象
											// json.groupname = groupname;
											json.groupid = groupid;
											json.groupaim = groupaim;
											json.groupaims = groupaims;
											json.grouphonor = grouphonor;
											$.ajax({
												url : 'updategroup.group',
												type : 'GET',
												data : json,
												success : function(){
													alert("成功更新数据~");
								...
								outStr2 += '<div class="team_title"><h1><a href="team1.jsp?id='+jsonCell.groupid+'">'+jsonCell.groupname+'</a></h1></div><div class="team_info"><div class="img_kuang"></div><img  id="team_img" src="img/tx/'+jsonCell.groupp+'" alt=""><div class="team_work"><h5>'+jsonCell.groupaim+'，大体进度安排：</h5><textarea style="width:400px;height:100px">'+jsonCell.groupaims+'</textarea></div><div class="team_chart"><h5 style="color: #f58220;font-weight: bold;">近七日进度</h5><canvas id="myChart2" width="400" height="400" style="background:#fff;float:left;"></canvas></div></div>';
	...
	//修改数据的ajax 						// @ajax：updatagroup.group
		// 小组的名字groupname，小组的阶段目标groupaim，小组目标的内容groupaims，小组荣誉grouphonor
			/*$("#XZZLbtn").click(
				function(){
