...
//0、导航栏
	// 加载头像及背景图片		 // @ajax：centerTX
		/*$.ajax({
			...
			error : function(){
				//传回数据格式：
				data = '{"list":[{"savedFileName":"img/bg3.jpg"]}';
		...
		// 头像savedFileName，昵称nickname，性别sex，成长分值point，个性签名personsign，个人荣誉personhonor，联系方式cotacti，小组编号groupid
				$.ajax({
					...
					success : function(data){
						...
						$.each(jsonArray,function(i,value){
							var jsonCell = value;
							outStr += '<tr><th>头　　像：</th><td><div id="fileimg"><div id="loading-cover"></div><div id="showloading"></div><img src="img/tx/';
							...
							if(jsonCell.sex == "1"){
								outStr += '<select name="" id=""  class="form-control"><option value="">不设置</option><option value="2">男</option><option value="1" selected="selected">女</option></select>';
							}
							else if(jsonCell.sex == "2"){
								outStr += '<select name="" id=""  class="form-control"><option value="">不设置</option><option value="2" selected="selected">男</option><option value="1">女</option></select>'
							}
							else{
								outStr += '<select name="" id=""  class="form-control" selected="selected"><option value="">不设置</option><option value="2">男</option><option value="1">女</option></select>';
							}
